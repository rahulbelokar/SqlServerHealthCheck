...
try
{
    ...
    {
        # Instance name passed as parameter from batch file.
        $SQLInstance = $args[0].replace('\','$')
    }
    ...
    # Database name passed as parameter from batch file.
    $DatabaseName = $args[1]
    ...
    if ($ver -eq 10)
    {
        $FolderPath = "$dir\Scripts_2008\"
    }
    elseif ($ver -eq 11)
    {
        $FolderPath = "$dir\Scripts_2012\"
    }
    elseif ($ver -eq 12)
    {
        $FolderPath = "$dir\Scripts_2014\"
    }
